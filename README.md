...
Long runs also survive a bad provider. Capacity errors, rate limits and 5xx are retried with jittered backoff; a string that still won't go through is left blank and the run carries on, so re-running the file retries exactly those and nothing else. If nothing is getting through at all — dead endpoint, wrong key, deprecated model — the run stops after 25 consecutive failures instead of grinding through thousands of doomed requests.

Translations are tuned for game text. The system prompt preserves `%s`, `{name}`, BBCode tags, line breaks, and capitalization style. A post-processing pass strips the usual LLM noise (`Translation:` prefixes, echoed source, surrounding quotes the source didn't have), and a line that comes back byte-identical to a 6+ word source is treated as a failed translation rather than shipped as English. Save the whole graph — nodes, connections, models, languages, parallel counts — to a JSON workflow under `addons/localization_ai/workflows/` and load it next session.
...
Architecture and design are mine; AI did a lot of the typing. Test coverage is one stdlib file (`python3 tests/test_translate_progress.py`) over the backend's failure paths, and there's no CI — so back up your `.csv` / `.po` files before pointing this at anything important and spot-check the output. LLMs occasionally drop placeholders or translate things they shouldn't. OpenRouter / Ollama costs and rate limits are yours; API keys live in a plaintext keyring under Godot's `user://`.
