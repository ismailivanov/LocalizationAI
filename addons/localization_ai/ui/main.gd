...
func _on_update_downloaded(result: int, code: int, _h: PackedStringArray, _b: PackedByteArray) -> void:
	...
	_about_status_lbl.text = "Installed %s. Restarting Godot…" % _about_latest_tag
	_about_install_btn.visible = false
	_about_open_btn.visible = false
	...
	done.dialog_text = ("LocalizationAI %s installed successfully.\n\n" \
			+ "Godot will now restart to apply the update.") % _about_latest_tag
	done.get_ok_button().text = "Restart Godot"
	var _do_restart := func() -> void:
		done.hide()
		EditorInterface.restart_editor(true)
	done.confirmed.connect(_do_restart)
	done.canceled.connect(_do_restart)
	add_child(done)
	done.popup_centered()
